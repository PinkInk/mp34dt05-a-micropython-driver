...
class wav:
    ...
    def __init__(self, filename, SampleRate=8000, BitsPerSample=8, NumChannels=1, NumSamples=0):
        ...
        try:
            ...
            if self.AudioFormat != 1: 
                raise TypeError('Compressed wav files not supported')
        except OSError:
            if not type(SampleRate) == type(BitsPerSample) == type(NumChannels) == int:
                raise TypeError('SampleRate, BitsPerSample & NumChannels must be integers')
    ...
    def SubChunk2Size(self):
        ...
        return len(self) * self.NumChannels * (self.BitsPerSample//8)
    ...
    def ByteRate(self):
        """Return ByteRate"""
        return self.SampleRate * self.NumChannels * (self.BitsPerSample//8)
    ...
    def write(self, overwrite=False):
        """Write file to disk"""
        # dirty, dirty hack (for WiPi?)
        # cwd = os.listdir()[0][0:3] if self.filename[0:3] not in ['0:/','1:/'] else ''
        # if cwd+self.filename in os.listdir() and not overwrite: 
        if self.filename in os.listdir() and not overwrite: 
            raise IOError('File exists')
        # file = io.open(cwd+self.filename,'wb')
        file = io.open(self.filename,'wb')
        header = b''
    ...
    def append_buffer(self, other):
        """Append frames as array of samples, of size BitsPerSample"""
        samplebytes = bytes(other)
        if len(samplebytes)%(self.BitsPerSample//8//self.NumChannels) != 0:
            raise ValueError('Invalid sample array size')
        self.data += samplebytes
